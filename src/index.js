...
// Only keep direct article links — skip Google News redirects
...
<div style="max-width:620px;margin:0 auto;padding:28px 16px">
  ...
  <div style="background:#0A0A0A;border-radius:16px;overflow:hidden;margin-bottom:12px">
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);text-align:center;padding:0 12px">
      ${[['#FFE600',leads.length,'Leads'],['#FF3B5C',highCount,'Hot'],['#FF6B2B',infraCount,'Infra'],['#1877F2',sponsorCount,'Sponsors'],['#777',fetchedCount,'Scanned']]
        .map(([color,val,label]) => `<div style="padding:16px 8px"><div style="font-family:'Syne',sans-serif;font-size:26px;font-weight:800;color:${color};line-height:1">${val}</div><div style="font-size:9px;color:#555;text-transform:uppercase;letter-spacing:1px;margin-top:3px">${label}</div></div>`).join('')}
    </div>
